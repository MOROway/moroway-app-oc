...
function changeCOSection(cO, isFront, input1, i, reverse) {
    ...
    if (realStandardDirection) {
        ...
        if (cO.state == 1 && Math.round(cO.x - background.x) >= Math.round(trains[input1].circle.x[1])) {
            ...
            cO.stateChange = true;
            cO.state = 211;
        } else if (cO.state == 212 && cO.y - background.y > 0 && isFirst) {
...
function placeTrainsAtInitialPositions(trainNumber) {
    trains.forEach(function (train, i) {
        if (trainNumber == undefined || trainNumber == i) {
...
function defineTrainParams() {
    ...
    rotationPoints.outer.rightSiding.end = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.continueCurve0 = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.continueLine0 = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.continueCurve1 = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.continueLine1 = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.continueCurve2 = {x: [], y: []};
    ...
    rotationPoints.outer.rightSiding.rejoin = {x: [], y: []};
