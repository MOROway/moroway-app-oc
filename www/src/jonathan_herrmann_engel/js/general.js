...
function handleServerJSONValues(key, func) {
    ...
    fetch(SERVER_JSON_LINK + "?key=" + key + "&platform=" + APP_DATA.platform + "&lang=" + CURRENT_LANG).then(function(response) {
        ...
        .catch(function(error) {
            console.log("Fetch-Imprint-Error:", error)
        })
...
function getSettings (asObject, storageArea){
    ...
    switch(storageArea) {
        case "morowayAppOC":
            dependencies = {"landscapeGame": ["landscapeApp"]};
            hardware = {};
    ...
    for(var i = 0; i < Object.keys(settings).length; i++) {
        ...
        if(getString("optTitle_" + storageArea + "_" + opt) != "undefined") {
            ...
            kid.addEventListener("click", function(event){
                changeSetting(event.target.parentNode.parentNode.dataset.settingsId);
            });
...
const STRINGS = {
    en:{
        ...
        optTitle_morowayAppOC_landscapeApp: "App: Allow screen rotation", optDesc_morowayAppOC_landscapeApp: "If disabled app always displays in landscape mode.", optTitle_morowayAppOC_landscapeGame: "Gameplay: Allow screen rotation", optDesc_morowayAppOC_landscapeGame: "If disabled game always displays in landscape mode.", optInfo_morowayAppOC_landscapeGame: "Limit orientation lock to gameplay.",
    ...
    de:{
        ...
        optTitle_morowayAppOC_landscapeApp: "App: Rotation erlauben", optDesc_morowayAppOC_landscapeApp: "Wenn nicht aktiviert, wird die App ausschließlich im Querformat dargestellt.", optTitle_morowayAppOC_landscapeGame: "Spiel: Rotation erlauben", optDesc_morowayAppOC_landscapeGame: "Wenn nicht aktiviert, wird das Spiel ausschließlich im Querformat dargestellt.", optInfo_morowayAppOC_landscapeGame: "Nur die Spielansicht auf Querformat beschränken.", 
