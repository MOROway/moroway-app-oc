...
function setSettingsHTML(elem, standalone){
    ...
    function changeSetting() {
        var settings = getSettings(false);
        if(isSettingActive(event.target.parentNode.parentNode.id)) {
            settings[event.target.parentNode.parentNode.id] = !settings[event.target.parentNode.parentNode.id];
    ...
    for(var i = 0; i < Object.keys(settings).length; i++) {
        ...
        if(getString("optTitle_" + opt) != "undefined") {
            ...
            kid.addEventListener("click", function(event){
                changeSetting(event.target.parentNode.parentNode.id);
            });
            child.appendChild(kid);
            kid = document.createElement("span");
            kid.textContent = getString("optTitle_" + opt);
            ...
            if(getString("optDesc_" + opt) != "undefined") {
                kid = document.createElement("span");
                kid.textContent = getString("optDesc_" + opt);
                child.appendChild(kid);
            }
            if(getString("optDesc_" + opt) != "undefined" && getString("optInfo_" + opt) != "undefined") {
                kid = document.createElement("br");
                child.appendChild(kid);
            }
            if(getString("optInfo_" + opt) != "undefined") {
                kid = document.createElement("i");
                kid.textContent = getString("optInfo_" + opt);
                child.appendChild(kid);
            }
            optElem.appendChild(child);
            if(opt == "saveGame") {
                ...
                kid.textContent = getString("optButton_saveGame_delete");
...
const STRINGS = {
    en:{
        ...
        platformOcStartGame: "Play", platformOcStartGameOnline: "Multiplayer", platformOcStartHelp: "Help/Legal", platformOcStartSettings: "Settings", platformOcStartSettingsOc: "Settings (Advanced)", platformOcSettingsOptLandscapeAppTitle: "App: Allow screen rotation", platformOcSettingsOptLandscapeAppDesc: "If disabled app always displays in landscape mode.", platformOcSettingsOptLandscapeGameTitle: "Gameplay: Allow screen rotation", platformOcSettingsOptLandscapeGameDesc: "If disabled game always displays in landscape mode.", platformOcSettingsOptLandscapeGameInfo: "Limit orientation lock to gameplay.", platformOcGameLeave: "Leave and destroy game?", platformOcGameLeaveTitel: "Are you sure?", platformOcGameLeaveYes: "Leave!", platformOcGameLeaveNo: "Cancel!", platformOcAppUpdate: "New features available!", platformOcAppUpdateTitel: "App-Update", platformOcAppUpdateYes: "View", platformOcAppUpdateNo: "Ignore", platformWindowsLinkError: "Error! Couldn't open link.", platformWindowsAppScreenFeedback: "Feedback", platformWindowsAppScreenChangelog: "Changelog"
    },
    de:{
        langName: "Deutsch",
        ...
        optTitle_showNotifications: "Benachrichtigungen", optDesc_showNotifications: "Zeigt Texteinblendungen bei verschiedenen Aktionen an.", optInfo_showNotifications: "Funktioniert nicht auf kleinen Bildschirmen. Einige Benachrichtigungen werden trotz Deaktivierung dieser Option angezeigt.",optTitle_classicUI: "Klassische Steuerelemente", optDesc_classicUI: "Zeigt die klassischen Steuerelemente wie Trafo und Zugauswahlbutton.", optTitle_alwaysShowSelectedTrain: "Ausgewählter Zug", optDesc_alwaysShowSelectedTrain: "Zeigt den Namen des ausgewählten Zuges immer an.", optInfo_alwaysShowSelectedTrain: "Erfodert aktivierte \"Klassische Steuerelemente\".", optTitle_cursorascircle: "Cursor als Farbe darstellen", optDesc_cursorascircle: "Zeigt statt dem Maussymbol einen Farbkreis.", optInfo_cursorascircle: "Funktioniert nicht auf Touchscreens.", optTitle_burnTheTaxOffice: "Brennendes Finanzamt animieren", optDesc_burnTheTaxOffice: "Zeigt Animationen zum brennenden Finanzamt.", optTitle_saveGame: "Spiel speichern", optDesc_saveGame: "Speichert den Zustand des jeweils aktuellen Spieles für das nächste Spiel.", optInfo_saveGame: "Wird im Mehrspieler-Modus ignoriert.", optButton_saveGame_delete: "Gespeichertes Spiel jetzt löschen", optApply: "Einstellungen aktualisiert", optLangSelectInfo: "Sprache wählen (erfordert Neuladen)", optLangSelectChange: "Sprachwahl gespeichert - Erneut laden, um anzuwenden", optLangSelectChangeButton: "Neuladen",
